...
class RunRequest(BaseModel):
    ...
    mode: str = Field("kafka", pattern="^(sync|kafka)$")
...
def _partition_key(query: dict) -> str:
    p = query["params"]
    return str(p.get("zone_id") or p.get("zone_a") or "Z0")


# ============= ruta SÍNCRONA (Sistema Base sin Kafka) ============= #
...
def _delivery(err, msg):
    ...
    if err is not None:
        state.errors += 1
        log.debug(f"Entrega fallida: {err}")
    else:
        state.sent += 1


def _produce(producer: Producer, message: dict, key: str):
    payload = json.dumps(message)
    while True:
        try:
            producer.produce(TOPIC_MAIN, key=key, value=payload, callback=_delivery)
            break
        except BufferError:
            producer.poll(0.1)
    producer.poll(0)
...
async def _run_experiment(cfg: RunRequest):
    ...
    spike_arrival = (
        PoissonInterArrival(cfg.spike_rate_qps, seed=cfg.seed + 5)
        if cfg.spike_rate_qps else None
    )
    spiked = False
    ...
    if cfg.mode == "sync":
        queue = asyncio.Queue(maxsize=cfg.concurrency * 4)
    ...
    try:
        while not state.stop_flag.is_set():
            ...
            query = _build_query(zone_sel, query_sel, conf_sel, bin_sel, rng)
            message = _enrich(query)

            if cfg.mode == "sync":
                await queue.put(message)
            else:
                _produce(producer, message, _partition_key(query))

            produced += 1

            # cambiamos a mitad del experimento
            if (spike_arrival is not None and not spiked
                    and cfg.spike_at_sec is not None
                    and (now - state.start_time) >= cfg.spike_at_sec):
                arrival = spike_arrival
                spiked = True
                log.info(f"SPIKE activado: tasa -> {cfg.spike_rate_qps} qps")
        ...
        if cfg.mode == "sync":
            ...
            log.info("Producción terminada. Haciendo flush del productor Kafka...")
            await asyncio.get_event_loop().run_in_executor(None, producer.flush, 15)
    finally:
        if cfg.mode == "sync":
            for _ in workers:
            ...
            await asyncio.get_event_loop().run_in_executor(None, producer.flush, 10)
        elapsed = time.time() - state.start_time
        log.info(
            f"Experimento '{cfg.label}' ({cfg.mode}) terminado. Produced={produced} "
            f"Sent={state.sent} Errors={state.errors} Elapsed={elapsed:.1f}s "
            f"Throughput={produced / max(elapsed, 0.01):.1f} qps"
        )
        state.running = False
